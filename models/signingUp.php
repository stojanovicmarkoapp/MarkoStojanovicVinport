 ...
 header("Content-type: application/json");
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        try{
            ...
            else if(!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,50}$/",$password)){
                $errors[]="You have not filled the field for the password properly. Example: rickleS26";
            }
            ...
            if(count($errors)==0){
                if(emailAddressExists($emailAddress)){
                    $response = ["message"=>"There is an user with the same email address."];
                    echo json_encode($response);
                    http_response_code(200);
                }
                else if(usernameExists($username)){
                    $response = ["message"=>"There is an user with the same username."];
                    echo json_encode($response);
                    http_response_code(200);
                }
                else {
                    ...
                    $roleId = 2;
                    $insertingUser = insertUser($firstName,$lastName,$emailAddress,$username,$encodedPassword,$roleId);
